...
# RETURNS: _parse_time_str, find_assignment_row, run_automation
# THREAD_TARGETS: 
# HOTKEYS: 
# TK_BINDS: 
# COMMAND_BINDS: 
...
#     - D2LProcessor._parse_time_str()
#===============================================================================
#
# CRITICAL GLOBAL VARIABLES:
#
# logger:
#   Read by: automate_d2l, open_course, process_course, run_automation, _write_debug_report, _parse_time_str +5 more
#
#===============================================================================
#
...
#   automate_d2l() — reads D2L_BASE_URL, logger; calls RuntimeError, async_playwright, asyncio.Event, asyncio.sleep, logger.error, logger.info; no return value
#   open_course() — reads COURSE_URLS, logger; calls COURSE_URLS.get, RuntimeError, asyncio.sleep, current_url.startswith, logger.info, page.goto; no return value
#   process_course() — reads COURSE_URLS, logger; calls COURSE_URLS.get, RuntimeError, assignments.append, asyncio.sleep, csv_module.DictReader, current_url.startswith; no return value
#   run_automation() — reads logger; calls asyncio.run, logger.error, logger.info, logger.warning, self.automate_d2l, self.context.close; returns value
#   _write_debug_report() — reads LOGS_DIR, logger; calls datetime.now, f.write, logger.error, logger.info, open, os.makedirs; no return value
#   _parse_time_str() — reads logger; calls am_pm_match.group, int, len, logger.error, re.search, s.split; returns value
#   perform_date_updates() — reads logger; calls assignment_info.get, due_date_str.lower, due_time_str.lower, enumerate, len, logger.debug; no return value
#   find_assignment_row() — reads logger; calls assignment_name.lower, assignment_name.replace, clean_name.lower, elem.evaluate_handle, elem.text_content, enumerate; returns value
#   update_due_date() — reads logger; calls RuntimeError, due_link.click, logger.info, page.wait_for_selector, row.query_selector, self._parse_time_str; no return value
#   update_start_date() — reads logger; calls RuntimeError, logger.info, page.wait_for_selector, row.query_selector, self._parse_time_str, self.set_date_in_dialog; no return value
#   set_date_in_dialog() — reads logger; calls RuntimeError, asyncio.sleep, btn.click, checkbox.check, checkbox.is_checked, date_str.split; no return value
#===============================================================================
#
